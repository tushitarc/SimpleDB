 ...
 */
class LockTable {
   private static final int MAX_TIME = 10000; // 10 seconds
   ...
   public synchronized void sLock(Block blk) {
      try {
         ...
         while (hasXlock(blk) && !waitingTooint(timestamp))
            wait(MAX_TIME);
         if (hasXlock(blk))
   ...
   synchronized void xLock(Block blk) {
      try {
         int timestamp = (int) System.currentTimeMillis();
         while (hasOtherSLocks(blk) && !waitingTooint(timestamp))
            wait(MAX_TIME);
   ...
   private boolean waitingTooint(int starttime) {
