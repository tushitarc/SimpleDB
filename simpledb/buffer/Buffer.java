...
public class Buffer {
   ...
   /*
    * Adding access time for buffer for LRU implementation
    * @author Akshay Jain
    */
   private long lastAccess = 0;
   private long secondLastAccess = 0;
   
   public long getSecondLastAccess(){
	   	return secondLastAccess;
   }
    ...
    */
   void pin() {
	  //If not accessed earlier
	  if(lastAccess == 0L && secondLastAccess == 0L){
		  lastAccess = System.nanoTime();
	  }
	  else{
		  secondLastAccess = lastAccess;
		  lastAccess = System.nanoTime();
	  }
      pins++;
   }
    ...
    */
   //Returning block since we would be adding this new block to bufferpool
   Block assignToNew(String filename, PageFormatter fmtr) {
      flush();
      fmtr.format(contents);
      blk = contents.append(filename);
      pins = 0;
      //Returning new block to be added to buffer
      return blk;
   }
